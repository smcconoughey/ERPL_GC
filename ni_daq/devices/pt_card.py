...
class PTCard(BaseDevice):
    ...
    def _convert_no_tare(self, current_ma, channel):
        if channel in self.sensor_config:
            ...
            # Check if alternate calibration is active for this channel
            use_alt = self.use_alt_cal.get(channel, False)
            if use_alt and 'max_psi_alt' in cal:
                max_psi = cal.get('max_psi_alt', 10000.0)
            else:
    ...
    def set_calibration_mode(self, channel: int, use_alt: bool) -> Dict[str, Any]:
        ...
        Args:
            ...
            use_alt: True to use max_psi_alt, False for primary max_psi
            
        Returns:
            Dict with success status and current calibration info
        """
        if channel not in self.sensor_config:
            return {'success': False, 'message': f'Channel {channel} not configured'}
        
        self.use_alt_cal[channel] = use_alt
        cal = self.sensor_config[channel].get('calibration', {})
        
        active_max = cal.get('max_psi_alt') if use_alt else cal.get('max_psi')
        label = cal.get('max_psi_alt_label', 'Alt') if use_alt else 'Primary'
        
        sensor_name = self.sensor_config[channel].get('name', f'PT{channel}')
        logger.info(f"PT {sensor_name} (ch{channel}): Switched to {label} calibration (max_psi={active_max})")
    ...
    def get_calibration_status(self) -> Dict[str, Any]:
        ...
        for ch in range(self.channel_count):
            if ch in self.sensor_config:
                cal = self.sensor_config[ch].get('calibration', {})
                use_alt = self.use_alt_cal.get(ch, False)
                status[ch] = {
                    'name': self.sensor_config[ch].get('name', f'PT{ch}'),
                    'use_alt': use_alt,
                    'max_psi': cal.get('max_psi'),
                    'max_psi_alt': cal.get('max_psi_alt'),
                    'max_psi_alt_label': cal.get('max_psi_alt_label', 'Alt'),
                    'active_max_psi': cal.get('max_psi_alt') if use_alt else cal.get('max_psi')
                }
        return status
    ...
    def process_data(self, raw_data: List[List[float]]) -> Dict[str, Any]:
        ...
        for channel_idx, channel_data in enumerate(raw_data):
            ...
            use_alt = self.use_alt_cal.get(channel_idx, False)
            cal = self.sensor_config.get(channel_idx, {}).get('calibration', {})
            cal_label = cal.get('max_psi_alt_label', 'Alt') if use_alt else 'Primary'
