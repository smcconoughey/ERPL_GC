...
class LCCard(BaseDevice):
    ...
    def __init__(self, chassis: str, module_slot: int):
        ...
        # Track which channels use alternate calibration slope
        # Key: channel index, Value: True = use slope_alt, False = use primary slope
        self.use_alt_slope = {ch: False for ch in range(self.channel_count)}
    ...
    def set_calibration_mode(self, channel: int, use_alt: bool) -> Dict[str, Any]:
        """Set calibration mode for a channel.
        
        Args:
            channel: Channel index (0-based)
            use_alt: True to use slope_alt, False for primary slope
            
        Returns:
            Dict with success status and current calibration info
        """
        if channel not in self.lc_config:
            return {'success': False, 'message': f'Channel {channel} not configured'}
        
        self.use_alt_slope[channel] = use_alt
        cal = self.lc_config[channel].get('calibration', {})
        
        active_slope = cal.get('slope_alt') if use_alt else cal.get('slope')
        label = cal.get('slope_alt_label', 'Alt') if use_alt else 'Primary'
        
        lc_name = self.lc_config[channel].get('name', f'LC{channel}')
        logger.info(f"LC {lc_name} (ch{channel}): Switched to {label} calibration (slope={active_slope})")
    ...
    def get_calibration_status(self) -> Dict[str, Any]:
        """Get current calibration mode for all channels."""
        status = {}
        for ch in range(self.channel_count):
            if ch in self.lc_config:
                cal = self.lc_config[ch].get('calibration', {})
                use_alt = self.use_alt_slope.get(ch, False)
                status[ch] = {
                    'name': self.lc_config[ch].get('name', f'LC{ch}'),
                    'use_alt': use_alt,
                    'slope': cal.get('slope'),
                    'slope_alt': cal.get('slope_alt'),
                    'slope_alt_label': cal.get('slope_alt_label', 'Alt'),
                    'active_slope': cal.get('slope_alt') if use_alt else cal.get('slope')
                }
        return status
    ...
    def convert_to_lbf(self, v_per_v: float, channel: int) -> float:
        ...
        Supports primary and alternate calibration slopes via use_alt_slope setting.
        ...
        # Check if we should use alternate slope
        use_alt = self.use_alt_slope.get(channel, False)
        if use_alt and 'slope_alt' in cal:
            slope = cal.get('slope_alt')
        else:
            slope = cal.get('slope')
    ...
    def process_data(self, raw_data: List[List[float]]) -> Dict[str, Any]:
        ...
        for ch_idx in range(min(self.channel_count, len(raw_data))):
            ...
            # Include calibration mode info
            use_alt = self.use_alt_slope.get(ch_idx, False)
            cal = self.lc_config.get(ch_idx, {}).get('calibration', {})
            cal_label = cal.get('slope_alt_label', 'Alt') if use_alt else 'Primary'
            ...
            result.append({
                ...
                'status': status,
                'units': 'lbf',
                'cal_mode': 'alt' if use_alt else 'primary',
                'cal_label': cal_label
            })
