...
class DAQWebSocketServer {
        ...
        this.wss.on('connection', (ws, req) => {
            ...
            ws.on('message', (message) => {
                try {
                    ...
                    } else if (msg.action === 'set_lc_cal_mode' && typeof msg.channel === 'number') {
                        const result = this.setLCCalMode(msg.channel, msg.use_alt === true);
                        ws.send(JSON.stringify(result));
                    } else if (msg.action === 'get_lc_cal_status') {
                        const status = this.getLCCalStatus();
                        ws.send(JSON.stringify({ type: 'lc_cal_status', data: status }));
                    } else if (msg.action === 'set_pt_cal_mode' && typeof msg.channel === 'number') {
                        const result = this.setPTCalMode(msg.channel, msg.use_alt === true);
                        ws.send(JSON.stringify(result));
                    } else if (msg.action === 'get_pt_cal_status') {
                        const status = this.getPTCalStatus();
                        ws.send(JSON.stringify({ type: 'pt_cal_status', data: status }));
                    } else {
    ...
    setLCCalMode(channel, useAlt) {
        try {
            const cmdPath = path.join(__dirname, `lc_cal_ch${channel}.cmd`);
            fs.writeFileSync(cmdPath, `set_cal_mode:${channel}:${useAlt ? 'alt' : 'primary'}`);
            console.log(`LC channel ${channel} calibration set to ${useAlt ? 'alt' : 'primary'}`);
            return { success: true, message: `LC${channel} calibration switched to ${useAlt ? 'alternate' : 'primary'} slope.` };
        } catch (error) {
            console.error(`Failed to set LC channel ${channel} calibration:`, error);
            return { success: false, message: error.message };
        }
    }
    ...
    getLCCalStatus() {
        try {
            const statusPath = path.join(__dirname, 'lc_cal_status.json');
            if (fs.existsSync(statusPath)) {
                const raw = fs.readFileSync(statusPath, 'utf-8');
            ...
            console.error('Failed to get LC cal status:', error);
            return {};
        }
    }
    
    setPTCalMode(channel, useAlt) {
        try {
            const cmdPath = path.join(__dirname, `pt_cal_ch${channel}.cmd`);
            fs.writeFileSync(cmdPath, `set_cal_mode:${channel}:${useAlt ? 'alt' : 'primary'}`);
            console.log(`PT channel ${channel} calibration set to ${useAlt ? 'alt' : 'primary'}`);
            return { success: true, message: `PT${channel} calibration switched to ${useAlt ? 'alternate' : 'primary'} max_psi.` };
        } catch (error) {
            console.error(`Failed to set PT channel ${channel} calibration:`, error);
            return { success: false, message: error.message };
        }
    }
    ...
    getPTCalStatus() {
        try {
            const statusPath = path.join(__dirname, 'pt_cal_status.json');
            if (fs.existsSync(statusPath)) {
                const raw = fs.readFileSync(statusPath, 'utf-8');
                return JSON.parse(raw);
            }
            ...
            console.error('Failed to get PT cal status:', error);
            return {};
        }
    }
