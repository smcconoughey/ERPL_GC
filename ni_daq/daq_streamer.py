...
class DAQStreamer:
    ...
    def stream_daq_data_sync(self) -> None:
        ...
        while self.running:
            ...
            # Check for calibration mode change requests
            lc_cal_requests, pt_cal_requests = self._collect_cal_mode_requests()
            if lc_cal_requests or pt_cal_requests:
                self._apply_cal_mode_changes(lc_cal_requests, pt_cal_requests)
    ...
    def _collect_cal_mode_requests(self) -> Tuple[Dict[int, bool], Dict[int, bool]]:
        """Collect calibration mode change requests from command files.
        
        Returns:
            Tuple of (lc_requests, pt_requests) mapping channel index to use_alt flag
        """
        base_path = Path(__file__).parent
        lc_requests: Dict[int, bool] = {}
        pt_requests: Dict[int, bool] = {}
        
        for cmd_path in base_path.glob('lc_cal_ch*.cmd'):
            try:
                content = cmd_path.read_text().strip()
                # Expected format: set_cal_mode:channel:primary|alt
                parts = content.split(':')
                if len(parts) >= 3 and parts[0] == 'set_cal_mode':
                    channel = int(parts[1])
                    use_alt = parts[2].lower() == 'alt'
                    lc_requests[channel] = use_alt
            except Exception as e:
                logger.warning(f"Failed to parse LC cal mode command {cmd_path}: {e}")
            finally:
                try:
                    cmd_path.unlink()
                except FileNotFoundError:
        ...
        for cmd_path in base_path.glob('pt_cal_ch*.cmd'):
            try:
                content = cmd_path.read_text().strip()
                # Expected format: set_cal_mode:channel:primary|alt
                parts = content.split(':')
                if len(parts) >= 3 and parts[0] == 'set_cal_mode':
                    channel = int(parts[1])
                    use_alt = parts[2].lower() == 'alt'
                    pt_requests[channel] = use_alt
            except Exception as e:
                logger.warning(f"Failed to parse PT cal mode command {cmd_path}: {e}")
            finally:
                try:
                    cmd_path.unlink()
                except FileNotFoundError:
        ...
        return lc_requests, pt_requests

    def _apply_cal_mode_changes(self, lc_requests: Dict[int, bool], pt_requests: Dict[int, bool]) -> None:
        """Apply calibration mode changes to LC and PT card devices."""
        for device in self.devices:
            try:
                device_type = str(device.device_info.get('device_type', '')).lower()
            except Exception:
            ...
            if lc_requests and ('lc' in device_type or '9237' in device_type):
                for channel, use_alt in lc_requests.items():
                    if hasattr(device, 'set_calibration_mode'):
                        result = device.set_calibration_mode(channel, use_alt)
                        logger.info(f"LC cal mode change: ch{channel} -> {'alt' if use_alt else 'primary'}: {result}")
                
                # Write status file for server to read
                self._write_lc_cal_status(device)
            
            if pt_requests and ('pt' in device_type or '9208' in device_type):
                for channel, use_alt in pt_requests.items():
                    if hasattr(device, 'set_calibration_mode'):
                        result = device.set_calibration_mode(channel, use_alt)
                        logger.info(f"PT cal mode change: ch{channel} -> {'alt' if use_alt else 'primary'}: {result}")
                
                # Write status file for server to read
                self._write_pt_cal_status(device)

    def _write_lc_cal_status(self, lc_device) -> None:
        """Write LC calibration status to file for server access."""
        try:
            if hasattr(lc_device, 'get_calibration_status'):
                status = lc_device.get_calibration_status()
                status_path = Path(__file__).parent / 'lc_cal_status.json'
                with open(status_path, 'w') as f:
                    json.dump(status, f, indent=2)
        except Exception as e:
            logger.warning(f"Failed to write LC cal status: {e}")
    
    def _write_pt_cal_status(self, pt_device) -> None:
        """Write PT calibration status to file for server access."""
        try:
            if hasattr(pt_device, 'get_calibration_status'):
                status = pt_device.get_calibration_status()
                status_path = Path(__file__).parent / 'pt_cal_status.json'
                with open(status_path, 'w') as f:
                    json.dump(status, f, indent=2)
        except Exception as e:
            logger.warning(f"Failed to write PT cal status: {e}")
    ...
    def _queue_sample_rows(self, sample_map: Dict[str, List[List[Dict[str, Any]]]]) -> None:
        ...
        for sample_idx in range(max_samples):
            ...
            lcCal = ['primary'] * 4  # Default calibration modes
            if isinstance(lc_samples, list) and sample_idx < len(lc_samples):
                for entry in lc_samples[sample_idx]:
                    ...
                    if isinstance(idx, int) and 0 <= idx < 4:
                        ...
                        if 'cal_mode' in entry:
                            lcCal[idx] = entry['cal_mode']
    ...
    def start_logging(self) -> Dict:
        ...
        try:
            ...
            for i in range(4):
                lc_header.append(f'LC{i}_cal')  # Calibration mode: 'primary' or 'alt'
    ...
    def _flush_pending_logs(self) -> None:
        ...
        with self.log_lock:
            ...
            while self._pending_lc_rows and self.lc_log_writer is not None:
                ...
                row.extend(sample.get('lcCal', ['primary'] * 4))  # Calibration modes
