<script>
  ...
  import { activeTab, showToast, showLegend } from '../stores/ui.js';
  import { wsConnected, sendCommand } from '../stores/websocket.js';
  import { pidLoaded, pidFileName } from '../stores/pid.js';

  const tabs = [
    { id: 'pid', label: 'P&ID', icon: 'fa-project-diagram' },
    { id: 'legend', label: 'Legend', icon: 'fa-book' },
  ];
...
<style>
  ...
  .file-badge i { font-size: 12px; color: var(--green-600); }
